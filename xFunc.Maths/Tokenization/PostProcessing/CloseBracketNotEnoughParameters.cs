...
namespace xFunc.Maths.Tokenization.PostProcessing
{
    ...
    /// </summary>
    public class CloseBracketNotEnoughParameters : LexerPostProcessorBase
    {
        ...
        public override void Process(IList<IToken> tokens)
        {
            ...
            {
                var token = tokens[i];
                if (token is SymbolToken symbolToken && symbolToken.IsComma())
                {
                    var nextToken = GetNextToken(tokens, i);
                    if (nextToken is SymbolToken nextSymbolToken && nextSymbolToken.IsCloseSymbol())
                    {
